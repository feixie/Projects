...
m = 30; %biggest possible delay time (adjust to find optimal)
...
for k=2:len
    ...
    b_param(k) = sum(theta_k(2:m+1));
    b_len = length(theta_k(2:m+1));
    %d = (sum(theta_k(2:m+1).*(1:b_len)')/b_param(k)) - 1;
    d = (sum(theta_k(2:m+1).*(1:b_len)')/b_param(k)) - 2; %Bai says -1!!
    d_param(k) = uint8(d+0.5);
    %d_param(k)  = d;
    
   
    
    %alternate b and d formula:
    %[b_param(k), d_param(k)] = max(theta_k(2:m+1));
